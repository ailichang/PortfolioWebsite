var projects = {
       ...
       displayDetail:function(id){
           ...
           var category = id.slice(0, splitPos);
           var index = id.slice(splitPos+1); //get project corrent index
           
           var projectCategory = projects.vr;
           switch(category){
               case "vr-project":
           ...
           console.log(index);
           var msg = HTMLProjectTitle.replace("%time%",projectCategory[index].time);
           ...
           var row = "<tr id='%id%'><td>%entry%</td><td>%value%</td></tr>";
           for(var property in projectCategory[index]){
                if(property == "images" || property == "videos" || property == "icon" || property == "title" || property == "time" || property == "headline") continue;
           ...
           for(var i = 0; i < projectCategory[index].images.length; i++){
                var image = projectCategory[index].images[i];
                ...
                var indicator = HTMLSliderIndicator.replace("%id%", i);
